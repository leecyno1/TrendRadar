...
- 内置 Dashboard：支持 **报告查看（Reports）/ 数据浏览（Browse）/ 配置管理（Manage）**
- 通过 `manage.py` 命令控制启动/停止：`docker exec -it trend-radar python manage.py start_webserver`
- 访问地址：`http://localhost:8080/`（端口可配置）
- 报告直链：`/output/<path>`（仅允许常见静态文件后缀，默认屏蔽 `.db` 等）
- 建议公网部署时设置 `ADMIN_TOKEN`，保护 `/manage` 与 `/api/admin/*`

**⚙️ Dashboard 管理页（配置可视化）**

- 管理页提供两种方式修改配置：
  - **Quick Settings**：常用配置项表单化编辑（推送模式/存储/通知/平台列表等）
  - **Advanced YAML**：直接编辑 `config.yaml` 全量内容（高级用）
- 支持一键重置：恢复为镜像内置默认配置（会自动备份当前文件）
...
2. **设置 GitHub Secrets（必需 + 可选平台）**:
   ...
   **设置步骤：**
   ...
   1. **查看生成的 URL**：
      ...
      - 格式如：`https://hooks.slack.com/services/<workspace>/<channel>/<token>`
...
> 💡 **Web 服务器说明**：
> - Dashboard：`http://localhost:8080/`（Reports/Browse/Manage）
> - 报告页：`http://localhost:8080/reports`
> - 原始报告直链：`http://localhost:8080/output/index.html`
> - 端口可在 `.env` 文件中配置 `WEBSERVER_PORT` 参数（部分平台也可用 `PORT`）
> - 自动启动：在 `.env` 中设置 `ENABLE_WEBSERVER=true`
> - 公网建议设置 `ADMIN_TOKEN` 保护 `/manage` 与 `/api/admin/*`
...
http://localhost:8080/reports                   # 报告列表 + 预览
http://localhost:8080/output/index.html         # 最新汇总（原报告页）
http://localhost:8080/output/2025-xx-xx/html/当日汇总.html   # 指定日期汇总
